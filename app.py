...
def calculate_total_rent_current_month():
    ...
    # Safely calculate last day of current month
    import calendar
    _, last_day = calendar.monthrange(current_year, current_month)

    end_date = datetime.datetime(current_year, current_month, last_day)

    total_rent = db.session.query(db.func.sum(Transaction.rent_fee))\
                    .filter(Transaction.issue_date >= start_date,
...
def issue_book():
    if request.method == "POST":
        ...
        # Validate member ID
        if not memberid:
            flash("Please provide a member ID", "error")
        ...
        # Lookup book by title or ID
        book = db.session.query(Book, Stock).join(Stock).filter(Book.title.like(f'%{title}%')).first()
        if book is None:
            book = db.session.query(Book, Stock).join(Stock).filter(Book.id.like(title)).first()
        if book is None:
